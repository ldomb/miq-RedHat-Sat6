=begin
 method: List_Clusters.rb
 Description: Lists all clusters known to satellite6 
 Author: Laurent Domb <laurent@redhat.com>
 License: GPL v3 

 There is currently a bug in Sat6 which does not allow to list vmware
 Clusters. There I created variable vmcluster in the instance. 
-------------------------------------------------------------------------------
...
vmcluster = nil || $evm.object['vmcluster']
...
katello_url = nil || $evm.object['katellourl']
  ...
  if provider == "Vmware"
    dialog_hash["#{vmcluster}"] = "#{vmcluster}"

  elsif provider == "Ovirt"
    clusters['results'].each do |dc|
    dialog_hash[dc['id']] = dc['name']
    end
  else
  ...
  if dialog_hash.blank?
    ...
    dialog_hash[nil] = '< choose a vm cluster >'
  end
